...
/**
 * The bare starter intentionally ships without a medium Berth, so it cannot
 * honestly present the three medium onboarding manifests this legacy suite
 * exercises. The authored demo station has two complete Berths, their physical
 * service fixtures, and a real crew plan whose shortages remain visible.
 */
async function openPortOpsFixture(page: Page): Promise<void> {
  await page.goto('/?scenario=demo-station');
  await page.waitForFunction(() => window.__harnessReady === true, { timeout: 15_000 });
}
...
async function revealOpeningOffers(page: Page): Promise<void> {
  ...
  await page.locator('#open-port-dispatch').click();
  await expect(page.locator('#port-dispatch-modal')).toBeVisible();
  await expect(page.locator('.traffic-offer')).toHaveCount(3);
}

async function acceptPassenger(page: Page): Promise<void> {
  const offerId = await page.evaluate(() => {
    const snapshot = window.__harnessGetState().snapshot as {
      trafficOffers: Array<{ id: number; offerKind: string }>;
    };
    const passenger = snapshot.trafficOffers.find((offer) => offer.offerKind === 'passenger');
    if (!passenger) throw new Error('Authored fixture has no passenger offer.');
    return passenger.id;
  });
  const button = page.locator(
    `.traffic-offer[data-traffic-offer-id="${offerId}"] button[data-traffic-action="accept"]`
  );
  await expect(button, 'passenger offer should have an enabled physical-interface acceptance').toBeEnabled();
  await button.click();
  await page.locator('#close-port-dispatch').click();
  await expect(page.locator('#port-dispatch-modal')).toBeHidden();
}
...
test('port ops: authored berth fixture presents three distinct choices and no legacy core marker', async ({ page }) => {
  await openPortOpsFixture(page);
  await revealOpeningOffers(page);

  const offers = await page.evaluate(() => {
    const snapshot = window.__harnessGetState().snapshot as {
      trafficOffers: Array<{ id: number; offerKind: string; riskLabel: string }>;
    };
    return snapshot.trafficOffers.map(({ id, offerKind, riskLabel }) => ({ id, offerKind, riskLabel }));
  });
  expect(offers.map((offer) => offer.offerKind).sort()).toEqual(['freight', 'mixed', 'passenger']);

  const cardFor = (kind: string) => {
    const offer = offers.find((candidate) => candidate.offerKind === kind);
    if (!offer) throw new Error(`Authored fixture is missing its ${kind} offer.`);
    return page.locator(`.traffic-offer[data-traffic-offer-id="${offer.id}"]`);
  };
  await expect(cardFor('passenger').locator('.traffic-risk-cue')).toHaveText('LOW RISK');
  await expect(cardFor('passenger').locator('.traffic-cues')).toContainText('Meals');
  await expect(cardFor('freight').locator('.traffic-risk-cue')).toHaveText('GUARDED RISK');
  await expect(cardFor('freight').locator('.traffic-cues')).toContainText('Unload cargo');
  await expect(cardFor('freight').locator('.traffic-cues')).toContainText('Load cargo');
  await expect(cardFor('mixed').locator('.traffic-risk-cue')).toHaveText('GUARDED RISK');
  await expect(cardFor('mixed').locator('.traffic-cues')).toContainText('Meals');
  await expect(cardFor('mixed').locator('.traffic-cues')).toContainText('Drinks');
  await expect(cardFor('mixed').locator('.traffic-cues')).toContainText('Leisure');
  await expect(page.locator('#shift-brief')).toContainText('Commit frontage carefully');
  await expect(page.locator('.traffic-load-forecast')).toHaveCount(3);
...
test('port ops: optional city lenses and Dock pod construction are available', async ({ page }) => {
  ...
  const cleanliness = page.locator('button[data-diagnostic-overlay="sanitation"]');
...
test('port ops: passenger promise settles and keeps the berth and crew healthy', async ({ page }) => {
  await openPortOpsFixture(page);
  await revealOpeningOffers(page);
  await acceptPassenger(page);
  await expect(page.locator('#shift-brief')).toContainText('SHORT 1 Cook');
  await expect(page.locator('#shift-brief')).toContainText('Hire the role, adjust its watch, or hold this arrival');
  await page.getByRole('button', { name: 'Play', exact: true }).click();
  ...
  const settledState = await page.evaluate(() => window.__harnessGetState() as {
    snapshot: { portOps: { settlements: unknown[]; contracts: Array<{ status: string }> } };
  });
  expect(settledState.snapshot.portOps.settlements).toHaveLength(1);
  expect(['settled', 'departed']).toContain(settledState.snapshot.portOps.contracts[0]?.status);
  ...
  await expect(page.locator('#hud-crew')).toHaveText('18');
});
...
test('port ops: cargo fault alert focuses the affected equipment', async ({ page }) => {
  ...
  const cargoAlert = page.locator('#alert-list button[title^="Primary cargo arm fault"]');
  await expect(cargoAlert).toContainText('Primary cargo arm fault');
  await cargoAlert.click();
