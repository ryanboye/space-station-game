...
- **Contracted-but-dead surfacing assets:** `AGENT_OVERLAY_SPRITE_KEYS` (distressed/critical/agitated) have zero atlas frames and no draw call; `fx.low_oxygen` is the inverse and was **misreported in this review** — corrected 2026-07-23: its draw call is live in `render.ts` and renders a vector fallback; the art was never packed. Art-only gap, not a wiring gap.
