...
- **The "Day N | Cycle X" HUD string is render-time fiction** (`main.ts:1259`). The sim has no day concept. `state.cycleDuration = 15s` now only supports the cosmetic HUD slice and traffic-rate math; arrivals are jittered checks, not fixed waves — and in the default port-ops mode (`manualTrafficAdmission`), player-admitted traffic offers. Don't add gameplay that depends on day boundaries.
